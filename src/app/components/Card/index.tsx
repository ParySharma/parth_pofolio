  ...
  &&.MuiCard-root {
    ...
    background-color: ${({ darkMode, backgroundColor }) =>
      darkMode
        ? '#333'
        : backgroundColor ||
          '#fff'}; // Change background color based on darkMode
    border-color: ${({ borderColor }) => borderColor || 'none'};
    color: ${({ darkMode, textColor }) =>
      darkMode
        ? '#fff'
        : textColor || '#111827'}; // Change text color based on darkMode
    border-radius: ${({ radius }) => radius || 12}px;
    font-size: ${({ fontSize }) => fontSize || 18}px;
    margin-top: ${({ marginTop }) => marginTop || 0}px;
    margin-left: ${({ ml }) => ml || 0}px;
    box-shadow: ${({ boxShadow, darkMode }) =>
      boxShadow ||
      (darkMode
        ? '0 4px 4px 0 #00000014'
        : 'none')}; // Change box-shadow based on darkMode
